...
class WeekData {
	...
	public static function reloadWeekFiles(isStoryMode:Null<Bool> = false, weekSuffix:String = "base")
	{
		weeksList = [];
		weeksLoaded.clear();
		#if MODS_ALLOWED
		...
		var sexList:Array<String> = CoolUtil.coolTextFile(Paths.getPreloadPath('weeks/'+weekSuffix+'/weekList.txt'));
		for (i in 0...sexList.length) {
			for (j in 0...directories.length) {
				var fileToCheck:String = directories[j] + 'weeks/'+weekSuffix+'/' + sexList[i] + '.json';
		...
		for (i in 0...directories.length) {
			var directory:String = directories[i] + 'weeks/'+weekSuffix+'/';
